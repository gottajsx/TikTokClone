...
import { useUpdateRelationshipPreferences, useMyPreferences } from '@/hooks/usePreferences';
...
export default function RelationsScreen({ mode = 'onboarding', onSaved }: Props) {
  const [selected, setSelected] = useState<Set<RelationshipType>>(new Set());
  const { mutate, isPending } = useUpdateRelationshipPreferences();
  
  // 🔹 Charger les préférences existantes en mode edit
  const { data: preferences } = useMyPreferences(mode === 'edit');

  useEffect(() => {
    if (mode === 'edit' && preferences?.relation_type?.length) {
      setSelected(new Set(preferences.relation_type));
    }
  }, [preferences, mode]);
